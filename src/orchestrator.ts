#!/usr/bin/env node
// Iteration Engine — AI-powered iterative planning and implementation pipeline
...
import { setDryRun, DRY_RUN, CONFIG, resolveConfig } from "./config.js";
...
function runBootstrap(): void {
  ...
  if (!existsSync(CONFIG.masterPlanFile)) {
    console.error(
      `\nMaster plan not found: ${CONFIG.masterPlanFile}\n` +
        `Create a ${CONFIG.masterPlanFile} in the project root first.\n`,
    );
    process.exit(1);
  }
  ...
  console.log("\n" + "=".repeat(56));
  console.log("  Iteration Engine — Bootstrapping from Master Plan");
...
function runPipeline(): void {
  ...
  console.log("\n" + "=".repeat(56));
  console.log("  Iteration Engine — Running");
  console.log("=".repeat(56) + "\n");
...
Commands:
  bootstrap [--force]
      Read the master plan, scaffold milestone directories, and create state.json
...
Configuration:
  Place a pipeline.config.json in the project root to override defaults.
